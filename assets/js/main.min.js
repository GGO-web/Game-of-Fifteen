...
gameField.addEventListener("click", function (e) {
  ...
  if (t.classList.contains("game-field__cell") && "" != n) {
    ...
    isAdjacentCell(_e3) && (isFirstClick && (timer = setInterval(function () {
      timeLeft = +(timeLeft + .1).toFixed(1), countTimes.innerHTML = normalizeTimeLeft(String(timeLeft)) + "s";
    }, 100), isFirstClick = !1), moves++, countMoves.innerHTML = "" + moves, switchStates(_e3, emptyCellIndex), emptyCellIndex = _e3), isGameWin() && (stopTimer(), closeGame(), updateGameWinInfo(), gameWinModalWrapper.classList.remove("hidden"));
  }
