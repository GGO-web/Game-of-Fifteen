...
var normalizeTimeLeft = function normalizeTimeLeft(e) {
  if (!e.includes(".")) return e += ",00";
  var t = e.indexOf(".");
  var n = (e = e.substring(0, t) + "," + e.substring(t + 1)).length - t - 1;

  for (var _t = 0; _t < 2 - n; ++_t) {
    e += "0";
  }
  ...
  countTimes.innerHTML = "0,00s", timeLeft = 0;
},
    startTimer = function startTimer() {
  timer = setInterval(function () {
    timeLeft = +(timeLeft + .01).toFixed(2), countTimes.innerHTML = normalizeTimeLeft(String(timeLeft)) + "s";
  }, 10);
},
...
gameField.addEventListener("click", function (e) {
  ...
  if (t.classList.contains("game-field__cell") && "" != n) {
    ...
    isAdjacentCell(_e3) && (isFirstClick && (timer = setInterval(function () {
      timeLeft = +(timeLeft + .01).toFixed(2), countTimes.innerHTML = normalizeTimeLeft(String(timeLeft)) + "s";
    }, 10), isFirstClick = !1), moves++, countMoves.innerHTML = "" + moves, switchStates(_e3, emptyCellIndex), emptyCellIndex = _e3), isGameWin() && (stopTimer(), closeGame(), updateGameWinInfo(), gameWinModalWrapper.classList.remove("hidden"));
  }
  ...
  resetCountMoves(), resetTimeOnGame(), stopTimer(), isFirstClick = !0, closeGame(), gameField.innerHTML = "", gameFieldSizeWrapper.classList.remove("hidden");
}), gameWinCloseButton.addEventListener("click", function () {
  gameWinModalWrapper.classList.add("hidden"), resetCountMoves(), resetTimeOnGame(), gameShuffle(), openGame(), isFirstClick = !0;
});